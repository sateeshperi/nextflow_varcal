...
<hr>
<center>This is part 4 of 14 of a <a href="/nextflow/" target="_blank">Introduction to NextFlow</a>.</center>
<hr>
...
<pre>
Add the following to `lists.nf` and `nextflow run maps.nf`:
```
...
For , the method `each()` when applied to a map can take a closure with two arguments, to which it passes the key-value pair for each entry in the map object. 
...
**Create a new `conditional.nf`; add the following and `nextflow run conditional.nf`:**
